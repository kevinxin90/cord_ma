...
def load_cord(data, semantic_type, prefix, go_mapping, cc):
    ...
    for edge in data['edges']:
        node1 = edge['node1']['id']
        node2 = edge['node2']['id']
        if (node1.startswith('GO:') and node1 in cc) or node1.startswith('MOP'):
            node1_id = node1
            node1_prefix = node1.split(':')[0]
            if node1_id not in res:
                res[node1_id] = {
                    node1_prefix.lower(): node1_id,
                    "associated_with": [],
                    "@type": semantic_type
                }
        ...
        if (node2.startswith('GO:') and node2 in cc) or node2.startswith('MOP'):
            node2_id = node2
            node2_prefix = node2.split(':')[0]
            if node2_id not in res:
                res[node2_id] = {
                    node2_prefix.lower(): node2_id,
                    "associated_with": [],
                    "@type": semantic_type
                }
...
def load_data(data_folder):
    ...
    with open(go_mapping) as f2:
        ...
        cc = [k for k, v in go_mapping_data.items() if v == "MolecularActivity"]
        print('cc', cc)

    res = load_cord(kg_data, "MolecularActivity", 'go', go_mapping_data, cc)
